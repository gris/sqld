...
use axum::Json;
use axum::extract::{State, BodyStream, Path};
...
pub async fn run_admin_api<F: MakeNamespace>(
    ...
    let router = axum::Router::new()
        ...
        .route("/v1/namespaces/:namespace/create-with-dump", post(handle_create_namespace))
        .with_state(Arc::new(AppState { db_config_store, namespaces }));
...
async fn handle_get_config<F: MakeNamespace>(State(app_state): State<Arc<AppState<F>>>) -> Json<Arc<DatabaseConfig>> {
...
async fn handle_create_namespace<F: MakeNamespace>(
    ...
    match app_state.namespaces.create_with_dump(namespace.into(), dump).await {
        Ok(_) => Ok(()),
